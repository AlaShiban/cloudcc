...
export const orderPlaced = persist(
  new Topic<OrderPlaced>({ subscribers: "many", ordering: "none", delivery: "at_least_once" }),
  { id: "orderPlaced" },
);

/** Same shape. A separate topic rather than a "type" field on the first,
 *  because subscribers differ. */
export const courierAssigned = persist(
  new Topic<CourierAssigned>({ subscribers: "many", ordering: "none", delivery: "at_least_once" }),
  { id: "courierAssigned" },
);

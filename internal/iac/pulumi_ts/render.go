...
func (b Backend) Emit(req iac.Request) error {
	...
	files := map[string][]byte{
		...
		"package.json":  renderPackageJSON(req.Config, programImports(req)),
...
func renderIndex(req iac.Request) ([]byte, error) {
	...
	b.WriteString(header(req.Config, importsUsed(req.Program, order)))
	...
	for _, key := range order {
		...
		if key.Kind == aws.KindLambda || key.Kind == aws.KindECSTask || key.Kind == aws.KindK8sDeployment {
...
func renderResource(namer *varNamer, res ir.Resource, logicalName string) (string, error) {
	...
	// The third argument, when there is one. Options say how a resource is
	// managed rather than what it is -- which provider creates it, above all --
	// and a Kubernetes resource beside a pile of AWS ones needs to say so.
	opts := ""
	if len(res.Opts()) > 0 {
		rendered, err := namer.object(res.Opts(), 0)
		if err != nil {
			return "", err
		}
		opts = ", " + rendered
	}
	return fmt.Sprintf("const %s = new %s(%s, %s%s);\n\n",
		name, tmpl.Class, quote(logicalName), props, opts), nil
}
...
func unitEnvironments(req iac.Request, namer *varNamer) (map[string]string, error) {
	...
	for _, in := range req.Program.IntentsOfKind(config.KindExecutionUnit) {
		...
		// Anything the user set in cloudcc.yaml wins over a derived binding.
...
// importsUsed returns the module aliases this program's resources need, sorted.
//
// Derived rather than fixed, because a program that runs nothing on Kubernetes
// should not import the Kubernetes provider: an unused import is a dependency
// somebody has to install, and `npm install` in the generated project is
// already the slowest step of a deploy.
func importsUsed(p *ir.Program, order []ir.Key) []string {
	...
	for _, key := range order {
		...
		if tmpl, err := Lookup(res.Template()); err == nil && tmpl.Import != "" {
			seen[tmpl.Import] = true
		}
	}
...
// moduleFor maps a module alias to the package it is imported from.
var moduleFor = map[string]string{
	"aws": "@pulumi/aws",
	"k8s": "@pulumi/kubernetes",
}

func header(cfg *config.App, imports []string) string {
	var b strings.Builder
	fmt.Fprintf(&b, `// Generated by cloudcc for application %q. Do not edit: recompiling overwrites
// this file. To change what is generated, edit cloudcc.yaml in your source tree.
import * as pulumi from "@pulumi/pulumi";
`, cfg.App)
	for _, alias := range imports {
		if module, ok := moduleFor[alias]; ok {
...
func renderProject(cfg *config.App) []byte {
	return []byte(fmt.Sprintf(`# Generated by cloudcc.
name: %s
runtime: nodejs
description: Infrastructure compiled from the %s application by cloudcc.
`, cfg.App, cfg.App))
}

func renderStack(cfg *config.App) string {
	return fmt.Sprintf(`# Stack settings for %s. cloudcc creates this file once and never overwrites it,
...
// packageVersions pins the provider packages, keyed by module alias. A version
// here and an import there have to agree, so they are read from one place.
var packageVersions = map[string]string{
	"aws": "^6.66.0",
	"k8s": "^4.18.0",
}

// programImports is the module aliases the emitted index.ts will use. Computed
// twice -- here and inside renderIndex -- rather than threaded through, because
// the alternative is an Emit that builds the index before it can name the files
// it is writing.
func programImports(req iac.Request) []string {
	order, err := req.Program.ResourceOrder()
	if err != nil {
		// The index render below will fail on the same cycle and say so
		// properly; guessing every provider here would only add noise to it.
		return []string{"aws"}
	}
	return importsUsed(req.Program, order)
}

func renderPackageJSON(cfg *config.App, imports []string) []byte {
	deps := []string{`        "@pulumi/pulumi": "^3.144.0"`}
	for _, alias := range imports {
		module, ok := moduleFor[alias]
		if !ok {
			continue
		}
		deps = append(deps, fmt.Sprintf("        %q: %q", module, packageVersions[alias]))
	}
	sort.Strings(deps)
...
`, quote(cfg.App), strings.Join(deps, ",\n")))
}

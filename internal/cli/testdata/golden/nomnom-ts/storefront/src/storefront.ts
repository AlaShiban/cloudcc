...
_cloudccExpose.register(app, { id: "nomnom-api" });

const orders = _cloudccKv.connect("orders") as DynamoDBClient;
...
const pricing = _cloudccRpc.connect("pricing");
const inventory = _cloudccRpc.connect("inventory");
const tracking = _cloudccRpc.connect("tracking");

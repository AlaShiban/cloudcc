// The HTTP half of a two-language application, in TypeScript.
//
// The two languages are the point, and TypeScript is the more interesting
// second one: nothing below is aware that the other unit is Python, and
// nothing above the store declarations is aware that this file is compiled at
// all. Run it as written with `npx tsx api.ts`.
//
...
const db = _cloudccOrm.connect("shopdb") as Pool;
...
const cache = _cloudccRedis.connect("petCache") as Redis;
...
const photos = _cloudccFs.connect("petPhotos") as S3Client;

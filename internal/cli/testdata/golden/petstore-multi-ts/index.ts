// Generated by cloudcc for application "petstore-multi-ts". Do not edit: recompiling overwrites
...
const petApiApi = new aws.apigatewayv2.Api("pet-api", {
    name: "petstore-multi-ts-pet-api",
    protocolType: "HTTP",
});
...
const petstoreTsSiteOai = new aws.cloudfront.OriginAccessIdentity("petstore-ts-site", {
    comment: "cloudcc petstore-multi-ts static unit petstore-ts-site",
});

const apiLogs = new aws.cloudwatch.LogGroup("api", {
    name: "/aws/lambda/petstore-multi-ts-api",
    retentionInDays: 14,
});

const workerLogs = new aws.cloudwatch.LogGroup("worker", {
    name: "/aws/lambda/petstore-multi-ts-worker",
    retentionInDays: 14,
});
...
const petsByOwnerTable = new aws.dynamodb.Table("petsByOwner", {
    ...
    hashKey: "id",
    name: "petstore-multi-ts-petsByOwner",
});
...
const apiRole = new aws.iam.Role("api", {
    ...
    name: "petstore-multi-ts-api-role",
});
...
const workerRole = new aws.iam.Role("worker", {
    ...
    name: "petstore-multi-ts-worker-role",
});

const petAuditBucket = new aws.s3.BucketV2("petAudit", {
    bucket: "petstore-multi-ts-petaudit",
    forceDestroy: true,
});

const petstoreTsSiteBucket = new aws.s3.BucketV2("petstore-ts-site", {
    bucket: "petstore-multi-ts-petstore-ts-site",
    forceDestroy: true,
});

const petstoreTsSiteBucketPolicy = new aws.s3.BucketPolicy("petstore-ts-site", {
    bucket: petstoreTsSiteBucket.id,
    ...
    Statement: [
        {
            ...
            Effect: "Allow",
            Principal: {
                AWS: petstoreTsSiteOai.iamArn,
            },
            Resource: [
                pulumi.interpolate`${petstoreTsSiteBucket.arn}/*`,
            ],
        },
    ],
    Version: "2012-10-17",
...
const petstoreTsSiteCdn = new aws.cloudfront.Distribution("petstore-ts-site", {
    comment: "cloudcc petstore-multi-ts static unit petstore-ts-site",
    defaultCacheBehavior: {
        ...
        maxTtl: 86400,
        minTtl: 0,
        targetOriginId: "s3-petstoreTsSite",
    ...
    origins: [
        {
            domainName: petstoreTsSiteBucket.bucketRegionalDomainName,
            originId: "s3-petstoreTsSite",
            s3OriginConfig: {
                originAccessIdentity: petstoreTsSiteOai.cloudfrontAccessIdentityPath,
            },
        },
    ],
...
const petstoreTsSiteIndexHtmlObject = new aws.s3.BucketObject("petstore-ts-site/index.html", {
    bucket: petstoreTsSiteBucket.id,
    contentType: "text/html; charset=utf-8",
    key: "index.html",
    source: new pulumi.asset.FileAsset("static/petstore-ts-site/index.html"),
});

const petstoreTsSiteStyleCssObject = new aws.s3.BucketObject("petstore-ts-site/style.css", {
    bucket: petstoreTsSiteBucket.id,
    contentType: "text/css; charset=utf-8",
    key: "style.css",
    source: new pulumi.asset.FileAsset("static/petstore-ts-site/style.css"),
});

const auditKeySecret = new aws.secretsmanager.Secret("auditKey", {
    name: "petstore-multi-ts-auditKey",
    recoveryWindowInDays: 0,
});

const petEventsQueue = new aws.sqs.Queue("petEvents", {
    name: "petstore-multi-ts-petEvents",
...
const networkVpc = new aws.ec2.Vpc("network", {
    ...
    tags: {
        Name: "petstore-multi-ts-network",
    },
});
...
const network0Subnet = new aws.ec2.Subnet("network-0", {
    ...
    tags: {
        Name: "petstore-multi-ts-network-0",
    },
    vpcId: networkVpc.id,
});
...
const petCacheCache = new aws.elasticache.Cluster("petCache", {
    clusterId: "petstore-multi-ts-petcache",
    engine: "redis",
...
const auditdbDb = new aws.rds.Instance("auditdb", {
    ...
    engine: "postgres",
    identifier: "petstore-multi-ts-auditdb",
...
const workerPolicy = new aws.iam.RolePolicy("worker", {
    name: "petstore-multi-ts-worker-policy",
    policy: pulumi.jsonStringify({
    Statement: [
        {
            Action: [
...
const petsdbDb = new aws.rds.Instance("petsdb", {
    ...
    dbSubnetGroupName: rdsSubnetGroup.name,
    engine: "postgres",
    identifier: "petstore-multi-ts-petsdb",
    instanceClass: "db.t4g.micro",
    manageMasterUserPassword: true,
    publiclyAccessible: false,
    skipFinalSnapshot: true,
...
const apiPolicy = new aws.iam.RolePolicy("api", {
    name: "petstore-multi-ts-api-policy",
    policy: pulumi.jsonStringify({
    Statement: [
        {
            Action: [
...
const petApiPermission = new aws.lambda.Permission("pet-api", {
    ...
    statementId: "petstore-multi-ts-pet-api-invoke",
});
...
export const CLOUDCC_STATIC_PETSTORE_TS_SITE_BUCKET = pulumi.interpolate`${petstoreTsSiteBucket.bucket}`;
export const CLOUDCC_STATIC_PETSTORE_TS_SITE_URL = pulumi.interpolate`${petstoreTsSiteCdn.domainName}`;
...
export const petstoreTsSiteUrl = petstoreTsSiteCdn.domainName;

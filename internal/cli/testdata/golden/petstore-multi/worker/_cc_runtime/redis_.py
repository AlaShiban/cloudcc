...
def connect(id):
    ...
    host = _client.env("CC_REDIS_%s_ENDPOINT" % slug, "persist_redis", id)
    port = int(_client.env("CC_REDIS_%s_PORT" % slug, "persist_redis", id))
    tls = os.environ.get("CC_REDIS_%s_TLS" % slug, "") == "true"
